...
static constexpr int GENERATIONS = 2;
...
// /** Write efficient frontier (risk vs expected return) to an HTML file with Chart.js. */
void write_efficient_frontier_chart(const Population& frontier, const std::string& filename = "efficient_frontier.html") {
    if (frontier.empty()) return;

    // Sort by risk (x-axis) for a clean curve
    Population sorted = frontier;
    sort(sorted.begin(), sorted.end(),
         [](const Individual& a, const Individual& b) { return a.risk < b.risk; });
...
<body>
  <div style="max-width: 800px; margin: 2rem auto; font-family: system-ui, sans-serif;">
    ...
    <p style="color: #666;">Risk (x) vs Expected Return (y) &mdash; Pareto-optimal portfolios</p>
    <canvas id="frontierChart" width="400" height="300"></canvas>
  </div>
  <script>
    ...
    for (size_t i = 0; i < sorted.size(); ++i) {
        if (i) out << ", ";
        out << "{ x: " << sorted[i].risk << ", y: " << sorted[i].expectedReturn << " }";
    }
    out << R"(],
    ...
    out.close();
    if (out && DEBUG)
        cout << "Efficient frontier chart written to " << filename << endl;
}
...
int main(){
    ...
    // cout << validate_archive(archive) << endl;
